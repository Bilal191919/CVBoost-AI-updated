                        ...
                        <div className="mt-8 w-full space-y-6 text-left">
                          <div className="flex flex-col gap-2">
                            <div className="flex justify-between text-sm font-bold text-slate-700">
                              ...
                              <span className="text-indigo-600">{result.atsScoreBreakdown.keywordMatching}%</span>
                            </div>
                            <div className="w-full bg-slate-100 rounded-full h-2">
                              <div className="bg-indigo-500 h-2 rounded-full" style={{ width: `${result.atsScoreBreakdown.keywordMatching}%` }}></div>
                            </div>
                            {result.atsScoreBreakdown.keywordFeedback ? (
                              <p className="text-xs text-slate-600 mt-1 leading-relaxed">{result.atsScoreBreakdown.keywordFeedback}</p>
                            ) : (
                              <p className="text-xs text-slate-500 mt-1 leading-relaxed">Measures how well your CV matches standard industry keywords or the provided job description.</p>
                            )}
                            {result.atsScoreBreakdown.keywordSuggestions && result.atsScoreBreakdown.keywordSuggestions.length > 0 && (
                              <ul className="mt-1 space-y-1.5">
                                {result.atsScoreBreakdown.keywordSuggestions.map((sugg, i) => (
                                  <li key={i} className="text-xs text-slate-600 flex items-start gap-1.5 bg-indigo-50/50 p-2 rounded-lg border border-indigo-100/50">
                          ...
                          <div className="flex flex-col gap-2">
                            <div className="flex justify-between text-sm font-bold text-slate-700">
                              ...
                              <span className="text-emerald-600">{result.atsScoreBreakdown.formatting}%</span>
                            </div>
                            <div className="w-full bg-slate-100 rounded-full h-2">
                              <div className="bg-emerald-500 h-2 rounded-full" style={{ width: `${result.atsScoreBreakdown.formatting}%` }}></div>
                            </div>
                            {result.atsScoreBreakdown.formattingFeedback ? (
                              <p className="text-xs text-slate-600 mt-1 leading-relaxed">{result.atsScoreBreakdown.formattingFeedback}</p>
                            ) : (
                              <p className="text-xs text-slate-500 mt-1 leading-relaxed">Evaluates readability, structure, and absence of complex layouts that confuse ATS parsers.</p>
                            )}
                            {result.atsScoreBreakdown.formattingSuggestions && result.atsScoreBreakdown.formattingSuggestions.length > 0 && (
                              <ul className="mt-1 space-y-1.5">
                                {result.atsScoreBreakdown.formattingSuggestions.map((sugg, i) => (
                                  <li key={i} className="text-xs text-slate-600 flex items-start gap-1.5 bg-emerald-50/50 p-2 rounded-lg border border-emerald-100/50">
                          ...
                          <div className="flex flex-col gap-2">
                            <div className="flex justify-between text-sm font-bold text-slate-700">
                              ...
                              <span className="text-amber-600">{result.atsScoreBreakdown.achievements}%</span>
                            </div>
                            <div className="w-full bg-slate-100 rounded-full h-2">
                              <div className="bg-amber-500 h-2 rounded-full" style={{ width: `${result.atsScoreBreakdown.achievements}%` }}></div>
                            </div>
                            {result.atsScoreBreakdown.achievementsFeedback ? (
                              <p className="text-xs text-slate-600 mt-1 leading-relaxed">{result.atsScoreBreakdown.achievementsFeedback}</p>
                            ) : (
                              <p className="text-xs text-slate-500 mt-1 leading-relaxed">Rewards quantified impact over generic responsibilities. ATS systems look for numbers and metrics.</p>
                            )}
                            {result.atsScoreBreakdown.achievementsSuggestions && result.atsScoreBreakdown.achievementsSuggestions.length > 0 && (
                              <ul className="mt-1 space-y-1.5">
                                {result.atsScoreBreakdown.achievementsSuggestions.map((sugg, i) => (
                                  <li key={i} className="text-xs text-slate-600 flex items-start gap-1.5 bg-amber-50/50 p-2 rounded-lg border border-amber-100/50">
                  ...
                  >
                    {result.sectionFeedback && result.sectionFeedback.length > 0 && (
                      <div className="bg-white p-6 sm:p-8 rounded-3xl shadow-sm border border-slate-100">
                        ...
                        <div className="space-y-4">
                          {result.sectionFeedback.map((section, i) => (
                            <div key={i} className="border border-slate-100 rounded-2xl p-5 bg-slate-50 relative overflow-hidden group">
                              <div className="absolute top-0 left-0 w-1 h-full bg-gradient-to-b from-indigo-400 to-purple-600 opacity-0 group-hover:opacity-100 transition-opacity"></div>
                              <div className="flex justify-between items-center mb-3">
                                <h4 className="font-bold text-lg text-slate-800">{section.section}</h4>
                                <div className={cn(
                                  "px-3 py-1 rounded-full text-xs font-bold shadow-sm",
                                  section.score >= 80 ? "bg-emerald-100 text-emerald-700" :
                                  section.score >= 60 ? "bg-amber-100 text-amber-700" :
                                  "bg-red-100 text-red-700"
                                )}>
                                  Score: {section.score}/100
                                </div>
                              </div>
                              <p className="text-sm text-slate-700 mb-4 leading-relaxed">{section.feedback}</p>
                              {section.suggestions && section.suggestions.length > 0 && (
                                <div>
                                  <h5 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Tailored Suggestions</h5>
                                  <ul className="space-y-2">
                                    {section.suggestions.map((sugg, j) => (
                        ...
                        <div className="mt-4 flex flex-col gap-2">
                          ...
                          <div className="flex justify-end gap-3 mt-3">
                            <button
                              ...
                              className="flex-1 flex items-center justify-center gap-2 text-sm font-medium text-slate-700 bg-white border border-slate-300 hover:bg-slate-50 py-2.5 rounded-xl transition-colors focus:outline-none focus:ring-2 focus:ring-slate-500"
                            ...
                            <button
                              ...
                              className="flex-1 flex items-center justify-center gap-2 text-sm font-medium text-white bg-emerald-600 hover:bg-emerald-700 py-2.5 rounded-xl transition-colors shadow-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2"
                            ...
                            <div className="mb-4 space-y-2">
                              ...
                              <div className="grid grid-cols-3 gap-2 text-center">
                                <div className="bg-white p-2 rounded-lg border border-slate-100 shadow-sm">
                                  <div className="text-[10px] text-slate-500 uppercase tracking-wider font-semibold mb-1">Keywords</div>
                                  <div className="text-sm font-bold text-indigo-600">{item.atsScoreBreakdown.keywordMatching}%</div>
                                </div>
                                <div className="bg-white p-2 rounded-lg border border-slate-100 shadow-sm">
                                  <div className="text-[10px] text-slate-500 uppercase tracking-wider font-semibold mb-1">Format</div>
                                  <div className="text-sm font-bold text-emerald-600">{item.atsScoreBreakdown.formatting}%</div>
                                </div>
                                <div className="bg-white p-2 rounded-lg border border-slate-100 shadow-sm">
                                  <div className="text-[10px] text-slate-500 uppercase tracking-wider font-semibold mb-1">Impact</div>
                                  <div className="text-sm font-bold text-amber-600">{item.atsScoreBreakdown.achievements}%</div>
                                </div>
                              </div>
                            </div>
                          )}
